...
export class PostQueryRepository {
    ...
    async getAllPostsForBlogId(query: PostQueryPaginationDto, blogId: string, userId?: string): Promise<PaginationViewModel<PostViewModel[]>> {
        const posts = await this.postModel.aggregate([
          { $match: { blogId } },
          { $sort: { [query.sortBy]: query.sortDirection } },{$skip: (query.pageNumber - 1) * query.pageSize },
    ...
    async getAllPosts(query: PostQueryPaginationDto, userId: string | null = null): Promise<PaginationViewModel<PostViewModel[]>> {
        const posts = await this.postModel.aggregate([
            {
                $sort: {
                    [query.sortBy]:
